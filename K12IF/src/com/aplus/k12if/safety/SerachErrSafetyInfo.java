...
/**
 * ------------------------------------------------------------
 * <p>Title:@SerachErrSafetyInfo.java </p>
 * <p>Description: 
 *    查询带异常（迟到，早退）的考勤安全信息
 * </p>
 ...
 * ------------------------------------------------------------
 */
@AutoInterface(id = "IF00030002", versions = "V1000", des = "查询带异常（迟到，早退）的考勤安全信息接口")
public class SerachErrSafetyInfo extends NoneTransactionInterfaceBase {
	
	
	// 查询异常（迟到，早退）的考勤安全信息
	final static String QUERY_ERR_SAFETY = "SELECT s.am_in_date,s.am_out_date,s.pm_in_date,s.pm_out_date,s.date FROM k12_safety" +
			" s where s.student_id = '{0}' and HOUR(s.am_in_date) < 8 and HOUR(s.pm_out_date) > 17  limit {1},{2}";
	...
	public String execute(IFRequestMessage tmb) {
		...
		try {
			DbTable table =	db.executeQuery(StringUtils.format(QUERY_ERR_SAFETY, params));
